/**
 * Owner: register a new private box at a stadium.
 */
...
export default function BoxForm() {
  ...
  const onSubmit = async (values: BoxValues) => {
    try {
      await post<Box>("/api/my/boxes", { ...values, capacity: Number(values.capacity) });
      toast.success("Palco registrado.");
      form.reset({ stadium_id: "" });
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      ...
      <Button type="submit" disabled={form.formState.isSubmitting} className="btn-gold w-full py-5 rounded-sm text-sm">
        {form.formState.isSubmitting ? "Registrando..." : "Registrar Palco"}
      </Button>
    </form>
  );
}
